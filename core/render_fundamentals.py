# core/render_fundamentals.py - Using FMP API
import os
import requests
import time

class RenderFundamentalsFetcher:
    """Fetcher using FMP API - More reliable with better free tier"""
    
    API_KEY = os.environ.get("FMP_API_KEY")  # Changed from ALPHA_VANTAGE_KEY
    ...
    def get_batch_fundamentals(cls, tickers, delay=0.2):
        ...
        for i, ticker in enumerate(tickers, 1):
            ...
            # Small delay to be safe (FMP allows 300+ per minute)
            if i < len(tickers):
                time.sleep(delay)
